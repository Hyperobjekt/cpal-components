...
export const getLanguageForContext = (
  ...
  Object.keys(LANG)
    ...
    .map(k =>
      getLang(
        k,
        populateContext(contextValues, contextPrefix),
      ),
    )
 ...
 */
export const getDescriptionForVarName = (
  varName,
  value,
) => {
...
export const getLegendEndLabelsForVarName = (
  ...
  return isGap
    ? [
        getLang(
          `${langPrefix}LOW_${metric}_${demographic}`,
        ),
        getLang(
          `${langPrefix}HIGH_${metric}_${demographic}`,
        ),
      ]
    : [
...
export const getTooltipText = values => {
  const text = Object.keys(values).reduce(
    (str, varName) => {
      return (
        str +
        getDescriptionForVarName(varName, values[varName])
      )
    },
    '',
  )
...
export const getLangWithSingleOrNone = (num, prefix) =>
  getLang(prefix + '_' + getSingleOrNoneQuantifier(num), {
    num,
  })
